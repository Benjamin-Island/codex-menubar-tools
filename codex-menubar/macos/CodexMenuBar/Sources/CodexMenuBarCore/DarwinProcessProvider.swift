...
public final class DarwinProcessProvider: ProcessProviding, @unchecked Sendable {
    ...
    public func processSnapshots() throws -> [ProcessSnapshot] {
        try reader.allPIDs().compactMap(snapshot)
    }
    ...
    private func snapshot(pid: Int32) -> ProcessSnapshot? {
        do {
            ...
            do {
                ...
                guard let argumentZero = arguments.first, !argumentZero.isEmpty else { return nil }
    ...
    private func isSessionLog(_ path: String) -> Bool {
        ...
        let prefix = sessionsDirectoryPath.hasSuffix("/")
            ? sessionsDirectoryPath
            : sessionsDirectoryPath + "/"
        return standardized.hasPrefix(prefix) && standardized.hasSuffix(".jsonl")
    }
}
...
private struct LibProcProcessReader: DarwinProcessReading {
    func allPIDs() throws -> [Int32] {
        ...
        guard estimatedCount >= 0 else { throw callError("proc_listallpids", pid: nil) }
        ...
        guard count >= 0 else { throw callError("proc_listallpids", pid: nil) }
    ...
    func executablePath(pid: Int32) throws -> String {
        ...
        guard length > 0 else { throw callError("proc_pidpath", pid: pid) }
        return String(
    ...
    private func openFile(pid: Int32, fileDescriptor: Int32) -> DarwinOpenFile? {
        ...
        let result = withUnsafeMutablePointer(to: &info) { pointer in
            proc_pidfdinfo(pid, fileDescriptor, PROC_PIDFDVNODEPATHINFO, pointer, Int32(expectedSize))
        }
    ...
    private func decodeArguments(_ bytes: [UInt8], pid: Int32) throws -> [String] {
        ...
        guard argumentCount >= 0 else { throw LibProcError.malformedArguments(pid: pid) }
